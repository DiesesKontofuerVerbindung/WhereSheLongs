...
func _apply_responsive_layout() -> void:
	var view := _view_size()
	var scale_factor := minf(view.x / 960.0, view.y / 540.0)
...
func _layout_hand_bottle(view: Vector2) -> void:
	...
	# Fit jar into ~58% of the shorter side so it stays centered on phone/desktop.
	var target_jar_width := minf(view.x * 0.58, view.y * 0.72)
	var scale := target_jar_width / (tex_size.x * JAR_INTERIOR_NORM.size.x)
