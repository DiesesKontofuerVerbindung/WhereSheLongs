# Hand Checkbox Prototype v0.2

独立技术验证：`Webcam -> INDEX_FINGER_TIP -> 在线 ✓ phase -> Trial -> CSV/JSON -> 可复现实验`。鼠标点击圆圈始终能完成 Checkbox，但不会混进手势指标。
...
每个手势从指尖进入启动区时自动建立 Trial。默认目标是 20 个 positive + 20 个 negative；目标数在 `config.py` 中配置。程序终端会打印实时 Trial 与混淆矩阵摘要。

## 在线状态机

运行状态：`IDLE -> TRACKING -> DRAWING -> CHECKED/FAILED`。
...
阶段不会被单帧 jitter 直接打断：原始坐标先做指数平滑，短小位移不进入几何轨迹；明显反向、离开区域、手部丢失超时或绘制超时才会产生 `FAILED`。每个 phase 至少占一个更新帧，因此 UI 和 CSV 能看到完整过程。
