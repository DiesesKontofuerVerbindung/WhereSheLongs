...
func _ready() -> void:
	...
	if events.size() != 134 or type_counts != expected_type_counts:
	...
	if not failures.is_empty():
		...
		get_tree().quit(1)
		return
	print("MYSTIC_NIGHT_TEST_PASS events=134 source_bounds=(1,146) narration_lines=56 dialogue_lines=50 stage_directions=10 cgs=13 videos=3 video_ranges=37_60,89_94,105_133 camera_shots=9 interactions=1 endpoint=true art_scenes=1_to_12 source96=7_to_8_to_9 audio=3")
	get_tree().quit(0)
