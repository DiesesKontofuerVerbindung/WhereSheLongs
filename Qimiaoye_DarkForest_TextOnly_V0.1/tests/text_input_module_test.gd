...
func _ready() -> void:
	...
	module.debug_set_text_for_verification("我在想")
	if not module.is_interference_active() or not module.is_waiting_for_fan():
		failures.append("开始输入后，杂念没有进入聚拢 / Fan 等待状态")
	if not module.are_interference_entities_entering_from_edges():
		failures.append("杂念没有从屏幕四周进入")
	if not module.is_input_editable():
		failures.append("杂念聚拢途中不应立即阻断输入")
	_verify_interference_phrases(module.get_interference_phrases(), failures)
	if module.debug_submit_for_verification("我在想"):
		failures.append("杂念尚未驱散时错误完成了输入")
	if module.ingest_gesture("NotFan"):
		failures.append("非 Fan 手势被正式契约错误接受")
	if not module.ingest_gesture("Fan"):
		failures.append("第一波杂念未接受 Fan 驱散")
	await get_tree().create_timer(0.95).timeout

	if module.get_wave_cleared_count() != 1:
		failures.append("第一波杂念没有完整驱散")
	if module.is_interference_active():
		failures.append("第一波驱散后杂念仍处于阻塞状态")
	if not module.is_input_editable():
		failures.append("Fan 驱散后输入框没有恢复编辑")
	if _finished_count != 0:
		failures.append("驱散第一波后提前结束，玩家无法继续输入")

	module.debug_set_text_for_verification("我在想但大家都已经替我决定了")
	if module.get_wave_started_count() != 2 or not module.is_interference_active():
		failures.append("继续输入后杂念没有再次回来")
	for sample in _detector_samples():
		module.ingest_hand_sample(sample[0], sample[1], sample[2])
	await get_tree().create_timer(0.95).timeout

	if module.get_wave_cleared_count() != 2:
		failures.append("OpenCV Fan 样本没有驱散第二波杂念")
	if not module.is_input_editable():
		failures.append("第二波驱散后输入框没有再次恢复")
	if _finished_count != 0:
		failures.append("第二波驱散后提前结束")

	if not module.debug_submit_for_verification("我在想但大家都已经替我决定了"):
		failures.append("驱散杂念并继续输入后无法提交")
	...
	if int(_result.get("interference_waves_cleared", 0)) != 2:
		failures.append("完成结果没有记录两波杂念")
	...
	if failures.is_empty():
		print("TEXT_INPUT_INTERFERENCE_PASS source=157 empty_rejected=true max_length=120 phrases=4 entities=12 gather_from_edges=true opencv_fan=true waves=2 input_resumed=true finish_after_input=true raw_text_logged=false")
		get_tree().quit(0)
		return
