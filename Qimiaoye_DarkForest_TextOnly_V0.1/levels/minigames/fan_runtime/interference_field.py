...
class InterferenceField:
    ...
    def reset(self) -> None:
        ...
        for index in range(config.INTERFERENCE_TEXT_ENTITY_COUNT):
            ...
            grid_x = (column + 0.5) * self.width / grid_columns
            grid_y = grid_top + row * (grid_bottom - grid_top) / (grid_rows - 1)
            x = _clamp(
                grid_x + random.uniform(-34.0, 34.0),
                text_width / 2 + 12.0,
                self.width - text_width / 2 - 12.0,
            )
            y = _clamp(
                grid_y + random.uniform(-22.0, 22.0),
                44.0 + text_height / 2,
