...
var _left_ear: float = 0.0
var _right_ear: float = 0.0
var _left_blink: float = 0.0
var _right_blink: float = 0.0
...
func _on_detector_frame(payload: Dictionary) -> void:
	...
	if kind == "hello":
		var nested = payload.get("lastFrame", {})
	...
	_left_ear = float(frame.get("leftEar", 0.0))
	_right_ear = float(frame.get("rightEar", 0.0))
	_left_blink = float(frame.get("leftBlink", 0.0))
	_right_blink = float(frame.get("rightBlink", 0.0))
...
func _eye_label(state: String) -> String:
	match state:
		"open":
			return "睁眼"
		"closed":
...
func _refresh_debug() -> void:
	...
	var cam := "已连接" if _camera_connected else "启动中"
	...
	_debug.set_text(
		...
		% [
			cam,
			_rx_frames,
			"检测到" if _face_detected else "未检测到",
			_eye_label(_left_eye),
			_eye_label(_right_eye),
			_hold_seconds,
			config.hold_seconds,
