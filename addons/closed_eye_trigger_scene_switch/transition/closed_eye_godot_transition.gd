class_name ClosedEyeGodotTransition
extends "res://addons/closed_eye_trigger_scene_switch/transition/closed_eye_transition_handler.gd"
...
func play(scene_id: String, p_config) -> void:
	config = p_config
	var mode := "Fade"
	if p_config:
...
func _load_scene(scene_id: String) -> void:
	if scene_id.is_empty():
		push_error("Closed-eye child scene is empty")
		return
