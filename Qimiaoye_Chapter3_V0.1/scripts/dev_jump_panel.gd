...
## 开发者 DOCX 行回溯面板（单章节）。
##
## 章节三事件表有独立的 DOCX 行号空间，面板只注册本章，用于按行号快速跳转测试。

const META_KEY := "qimiaoye_chapter3_dev_docx_jump"
...
func verify_contract() -> bool:
    if _tab_bar == null or _input == null or _jump_button == null or _feedback == null:
        return false
    if _chapters.is_empty() or _tab_bar.tab_count != _chapters.size():
        return false
...
static func take_pending_jump(root: Window, self_chapter_id: String) -> Dictionary:
    ...
    var target_chapter := str(payload.get("chapter", self_chapter_id))
